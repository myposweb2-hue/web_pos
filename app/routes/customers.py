...
def _as_bool(value):
    """Convert common boolean-like values into a real bool."""
...
def customers():
    """Main customers page."""
    # Read persisted global orders setting so the template can reflect it
    try:
        orders_setting = Setting.query.filter_by(
            setting_category='orders',
            setting_key='show_all_orders'
        ).first()
        orders_show_all = bool(orders_setting and orders_setting.setting_value and orders_setting.setting_value.lower() == 'true')
    except Exception:
        orders_show_all = False

    return render_template('customers/customers.html', orders_show_all=orders_show_all)
...
def list_orders():
    ...
    # By default show only orders taken by the current user.
    # Super Admins may pass `show_all=true` to view all orders.
    show_all_param = request.args.get('show_all', '').strip().lower()
    is_super_admin = bool(getattr(current_user, 'role', '') and getattr(current_user, 'role', '').lower() == 'super admin')

    # Check for a global 'show_all_orders' setting; if enabled, it overrides
    # per-user scoping and shows all orders to all users.
    ...
    try:
        ...
        if orders_setting and orders_setting.setting_value:
            global_show_all = orders_setting.setting_value.lower() == 'true'
    except Exception:
        global_show_all = False

    # If no explicit "taken_by" filter is provided, apply user-scoped filter
    # unless the global setting is enabled or the current user is a Super Admin
    # who explicitly asked to show all orders via the request param.
    if not taken_by:
        effective_show_all = global_show_all or (is_super_admin and show_all_param == 'true')
        if not effective_show_all:
            try:
                # Ensure we only include orders taken by the current user
                query = query.filter(Sale.user_id == int(getattr(current_user, 'id', 0)))
            except Exception:
                # If current_user id is not available, fall back to no results
                query = query.filter(Sale.id == -1)
...
@customers_bp.route('/api/orders-settings', methods=['GET', 'POST'])
@login_required
@require_permission('can_access_customers')
def get_set_orders_settings():
    """Get or set global orders settings (Super Admin only may update)."""
    from app.models import Setting
    ...
    if request.method == 'POST':
        ...
        data = request.get_json() or {}
        show_all = bool(data.get('show_all_orders'))
        ...
        if setting:
            setting.setting_value = 'true' if show_all else 'false'
        else:
            setting = Setting(
                setting_category='orders',
                setting_key='show_all_orders',
                setting_value='true' if show_all else 'false'
            )
            db.session.add(setting)

        db.session.commit()

        return jsonify({'success': True, 'show_all_orders': show_all})
    ...
    return jsonify({'show_all_orders': (setting.setting_value.lower() == 'true') if setting else False})

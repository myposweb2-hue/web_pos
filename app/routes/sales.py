...
def receipt_html(sale_id):
    ...
    if not logo_setting:
        logo_setting = Setting.query.filter_by(
            setting_category='receipt', setting_key='logo_path', company_id=company_id
        ).first()
    if not logo_setting:
        logo_setting = Setting.query.filter_by(
    ...
    if not logo_setting:
        logo_setting = Setting.query.filter(
            Setting.setting_category == 'receipt', Setting.setting_key == 'logo_path',
            Setting.company_id.is_(None)
        ).order_by(Setting.id.desc()).first()
    if not logo_setting:
        logo_setting = Setting.query.filter(
...
# Duplicate `receipt_html_public` removed (kept canonical implementation earlier in file).

# Duplicate `download_receipt_pdf` implementation removed. Kept the canonical implementation earlier in this file.

...
def _product_upload_dir():
    upload_dir = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'uploads', 'products'))
...
def _product_image_url(filename):
    ...
    if not clean_name:
        return ''
    return f"/uploads/products/{clean_name}"


def _image_filename_from_path(image_path):
    if not image_path:
        return ''
    ...
    if value.startswith('/uploads/products/') or value.startswith('uploads/products/'):
        return os.path.basename(value)
    return os.path.basename(value)
...
@require_permission('can_view_inventory')
def export_products():
    """Export all products to a ZIP containing the Excel workbook and any product images."""
    try:
        company_id = get_company_id()
        ...
        upload_dir = _product_upload_dir()
        data = []
        image_files = {}
        for product in products:
            image_name = _image_filename_from_path(product.image_path)
            if image_name:
                image_source = os.path.join(upload_dir, image_name)
                if os.path.isfile(image_source):
                    image_files[image_name] = image_source
            data.append({
                'Name': product.name,
                'Price': product.price,
        ...
        df.to_excel(workbook_output, index=False, engine='openpyxl')
        workbook_output.seek(0)

        zip_buffer = io.BytesIO()
        with zipfile.ZipFile(zip_buffer, 'w', compression=zipfile.ZIP_DEFLATED) as archive:
            archive.writestr('products.xlsx', workbook_output.getvalue())
            for image_name, image_source in sorted(image_files.items()):
                archive.write(image_source, arcname=f'images/{image_name}')

        zip_buffer.seek(0)

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f'inventory_products_{timestamp}.zip'
        return send_file(
            zip_buffer,
...
@require_permission('can_edit_inventory')
def import_products():
    """Import products from Excel or ZIP export files, including bundled images."""
    ...
    try:
        ...
        ext = file.filename.rsplit('.', 1)[-1].lower() if '.' in file.filename else ''
        if ext not in allowed_extensions:
            return jsonify({'error': 'File type not allowed. Allowed: xlsx, xls, zip'}), 400

        upload_dir = _product_upload_dir()
        tmp_dir = tempfile.mkdtemp(prefix='inventory_import_')
        if ext == 'zip':
            ...
            for root, _, files in os.walk(tmp_dir):
                for name in files:
                    ...
                    if lower_name.endswith(('.xlsx', '.xls')):
                        spreadsheet_candidates.append(os.path.join(root, name))
            if not spreadsheet_candidates:
                return jsonify({'error': 'The ZIP file does not contain an Excel file to import'}), 400
            spreadsheet_file = sorted(spreadsheet_candidates)[0]
        else:
            spreadsheet_file = os.path.join(tmp_dir, f'imported_products.{ext}')
            with open(spreadsheet_file, 'wb') as temp_file:
        ...
        for root, _, files in os.walk(tmp_dir):
            for name in files:
                if name.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.webp')):
                    image_files_lookup[os.path.basename(name).lower()] = os.path.join(root, name)
    ...
    def normalize_image_value(value):
        ...
        if image_value.startswith('http://') or image_value.startswith('https://'):
            return image_value
        if image_value.startswith('/uploads/products/'):
            return image_value
        if image_value.startswith('uploads/products/'):
            return '/' + image_value
        return image_value

    def save_imported_image(image_value):
        if not image_value:
            return None
        ...
        source_path = None
        if ext == 'zip':
            source_path = image_files_lookup.get(lower_name)
            if not source_path:
                for key, path in image_files_lookup.items():
                    if key == lower_name or os.path.basename(path).lower() == lower_name:
                        source_path = path
                        break

        if source_path and os.path.isfile(source_path):
            destination_name = f"{uuid.uuid4().hex}{os.path.splitext(file_name)[1].lower()}"
            destination_path = os.path.join(upload_dir, destination_name)
            shutil.copy2(source_path, destination_path)
            return _product_image_url(destination_name)

        existing_path = os.path.join(upload_dir, file_name)
        if os.path.isfile(existing_path):
            return _product_image_url(file_name)
    ...
    for index, row in df.iterrows():
        try:
            ...
            image_path_value = normalize_image_value(row.get('Image Path'))
            product_image_path = save_imported_image(image_path_value)
            ...
            if existing_product:
                ...
                if product_image_path:
                    existing_product.image_path = product_image_path
...
@inventory_bp.route('/api/products/export-zip')
@csrf.exempt
@login_required
@require_permission('can_view_inventory')
def export_products_zip():
    """Export all products to ZIP file (including images)."""
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            export_dir = os.path.join(temp_dir, f"inventory_export_{uuid.uuid4().hex}")
            os.makedirs(export_dir, exist_ok=True)
            
            # Copy product images to the export directory
            for product in products:
                if product.image_path:
                    image_filename = _image_filename_from_path(product.image_path)
                    if image_filename:
                        src_path = os.path.join(_product_upload_dir(), image_filename)
                        dst_path = os.path.join(export_dir, image_filename)
            ...
            # Export Excel file to the export directory
            excel_path = os.path.join(export_dir, "inventory_products.xlsx")
            df.to_excel(excel_path, index=False, engine='openpyxl')
            
            # Create a ZIP file containing the Excel file and images
            zip_path = os.path.join(temp_dir, f"inventory_export_{uuid.uuid4().hex}.zip")
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                # Add the Excel file
                zipf.write(excel_path, arcname="inventory_products.xlsx")
                ...
                for product in products:
                    if product.image_path:
                        image_filename = _image_filename_from_path(product.image_path)
                        if image_filename:
                            image_src_path = os.path.join(_product_upload_dir(), image_filename)
                            image_dst_path = os.path.join("images", image_filename)
                            zipf.write(image_src_path, arcname=image_dst_path)
        ...
        import traceback
        from flask import current_app
        current_app.logger.error(f"Export ZIP error: {str(e)}\n{traceback.format_exc()}")
        return jsonify({'error': str(e)}), 500

@inventory_bp.route('/api/products/import-zip', methods=['POST', 'OPTIONS'])
@csrf.exempt
@login_required
@require_permission('can_edit_inventory')
def import_products_zip():
    """Import products from ZIP file (including images)."""
    # Handle OPTIONS requests (browser preflight)
    if request.method == 'OPTIONS':
        return '', 204
    ...
    try:
        ...
        if ext not in allowed_extensions:
            return jsonify({'error': 'File type not allowed. Allowed: zip'}), 400
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Find the Excel file in the extracted contents
            excel_path = None
        ...
        import traceback
        from flask import current_app
        error_msg = f"Products import ZIP error: {str(e)}\n{traceback.format_exc()}"

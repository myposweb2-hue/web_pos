...
from app.models import db, Sale, SaleItem, Product, Customer, InventoryTransaction, User, Return, ReturnItem, Expense, CustomerPayment, CashierShift, Cheque, ChequeDeposit
...
@reports_bp.route('/api/cheques-summary')
@login_required
@require_permission('can_view_reports')
def get_cheques_summary():
    """Return a summary of cheques by status and top banks for a date range."""
    company_id = get_company_id()
    start_date = request.args.get('start_date')
    end_date = request.args.get('end_date')
    ...
    query = Cheque.query.filter(and_(Cheque.cheque_date >= start, Cheque.cheque_date <= end))
    if company_id and hasattr(Cheque, 'company_id'):
        query = query.filter(Cheque.company_id == company_id)

    total_count = query.count()
    total_amount = db.session.query(func.sum(Cheque.amount)).filter(and_(Cheque.cheque_date >= start, Cheque.cheque_date <= end))
    if company_id and hasattr(Cheque, 'company_id'):
        total_amount = total_amount.filter(Cheque.company_id == company_id)
    total_amount = float(total_amount.scalar() or 0)

    # Group by status
    status_query = db.session.query(Cheque.status, func.count(Cheque.id), func.sum(Cheque.amount))
    status_query = status_query.filter(and_(Cheque.cheque_date >= start, Cheque.cheque_date <= end))
    if company_id and hasattr(Cheque, 'company_id'):
        status_query = status_query.filter(Cheque.company_id == company_id)
    status_groups = status_query.group_by(Cheque.status).all()

    # Top banks by amount
    bank_query = db.session.query(Cheque.bank_name, func.count(Cheque.id), func.sum(Cheque.amount))
    bank_query = bank_query.filter(and_(Cheque.cheque_date >= start, Cheque.cheque_date <= end))
    if company_id and hasattr(Cheque, 'company_id'):
        bank_query = bank_query.filter(Cheque.company_id == company_id)
    top_banks = bank_query.group_by(Cheque.bank_name).order_by(desc(func.sum(Cheque.amount))).limit(10).all()
    ...
    return jsonify({
        ...
        'by_status': [
            {'status': s[0], 'count': int(s[1] or 0), 'amount': float(s[2] or 0)} for s in status_groups
        ],
        'top_banks': [
            {'bank': b[0], 'count': int(b[1] or 0), 'amount': float(b[2] or 0)} for b in top_banks
        ]
    })


@reports_bp.route('/api/cheque-list')
@login_required
@require_permission('can_view_reports')
def get_cheque_list():
    """Return a list of cheques filtered by parameters for the reports UI."""
    company_id = get_company_id()
    ...
    if start_date and end_date:
        ...
        query = query.filter(and_(Cheque.cheque_date >= start, Cheque.cheque_date <= end))

    if status:
        query = query.filter(Cheque.status == status)
    if bank:
        query = query.filter(Cheque.bank_name.ilike(f"%{bank}%"))
    if company_id and hasattr(Cheque, 'company_id'):
        query = query.filter(Cheque.company_id == company_id)

    query = query.order_by(Cheque.cheque_date.desc())
    ...
    for c in items:
        results.append({
            ...
            'branch': c.branch,
            'cheque_date': c.cheque_date.strftime('%Y-%m-%d') if c.cheque_date else None,
            'amount': float(c.amount or 0),
            'payer_name': c.payer_name or (c.customer.name if c.customer else None),
    ...
    return jsonify({'cheques': results, 'page': page, 'per_page': per_page})
...
def export_report_excel(report_type):
    ...
    elif report_type == 'cheques':
        # Cheques report
        ws = workbook.add_worksheet('Cheques')
        ws.write('A1', 'Cheques Report', bold_format)
        ws.write('A2', f'Period: {start_date} to {end_date}')

        ws.write('A4', 'ID', header_format)
        ws.write('B4', 'Cheque No', header_format)
        ws.write('C4', 'Bank', header_format)
        ws.write('D4', 'Branch', header_format)
        ws.write('E4', 'Cheque Date', header_format)
        ws.write('F4', 'Amount', header_format)
        ws.write('G4', 'Payer', header_format)
        ws.write('H4', 'Customer', header_format)
        ws.write('I4', 'Sale ID', header_format)
        ws.write('J4', 'Status', header_format)

        # Query cheques in date range
        cheque_query = Cheque.query.filter(and_(Cheque.cheque_date >= start, Cheque.cheque_date <= end))
        company_filter = get_company_filter(Cheque)
        if company_filter is not None:
            cheque_query = cheque_query.filter(company_filter)

        cheques = cheque_query.order_by(desc(Cheque.cheque_date)).all()

        row = 4
        for c in cheques:
            ws.write(row, 0, c.id)
            ws.write(row, 1, c.cheque_number)
            ws.write(row, 2, c.bank_name)
            ws.write(row, 3, c.branch or '')
            ws.write(row, 4, c.cheque_date.strftime('%Y-%m-%d') if c.cheque_date else '')
            ws.write(row, 5, float(c.amount or 0), money_format)
            ws.write(row, 6, c.payer_name or (c.customer.name if c.customer else ''))
            ws.write(row, 7, c.customer.name if c.customer else '')
            ws.write(row, 8, c.sale_id)
            ws.write(row, 9, c.status)
            row += 1

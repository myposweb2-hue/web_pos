...
def import_company_data_from_sql(company_id, sql_data):
    ...
    from sqlalchemy.sql.sqltypes import DateTime as SQLDateTime, Date as SQLDate
    ...
    for table_name, table_data in data.get('tables', {}).items():
        ...
        try:
            ...
            # Insert records
            mapper = inspect(model)
            column_types = {column.name: column.type for column in mapper.columns}
            for record in table_data:
                # Ensure company_id matches
                if 'company_id' in record:
                    record['company_id'] = company_id
                # JSON exports serialize date/datetime values as ISO strings.
                # Convert them back to Python values before SQLAlchemy inserts.
                normalized_record = dict(record)
                for column_name, column_type in column_types.items():
                    value = normalized_record.get(column_name)
                    if not isinstance(value, str):
                        continue
                    try:
                        if isinstance(column_type, SQLDateTime):
                            normalized_record[column_name] = datetime.fromisoformat(value.replace('Z', '+00:00')).replace(tzinfo=None)
                        elif isinstance(column_type, SQLDate):
                            normalized_record[column_name] = date.fromisoformat(value[:10])
                    except (TypeError, ValueError):
                        current_app.logger.warning(f'[RESTORE] Could not parse {table_name}.{column_name}: {value}')
                # Create instance and add
                obj = model(**normalized_record)
                db.session.add(obj)
...
def _resolve_sqlite_database_path():
    """Resolve the actual SQLite file used by SQLAlchemy/Flask."""
    database = getattr(getattr(db, 'engine', None), 'url', None)
    ...
    candidates = [
        ...
        os.path.join(os.path.abspath(os.path.dirname(current_app.root_path)), database),
...
def create_full_backup():
    ...
    try:
        ...
        if db_uri.startswith('sqlite:'):
            source_db = _resolve_sqlite_database_path()
            if not source_db or not os.path.isfile(source_db):
                return jsonify({'error': 'SQLite database file was not found', 'checked_path': source_db}), 500
            ...
            dump_cmd = [find_pg_dump(), f'--host={parsed_uri.hostname or "localhost"}', f'--port={parsed_uri.port or 5432}', f'--username={parsed_uri.username or "postgres"}', '--format=plain', f'--file={database_path}', parsed_uri.path.lstrip('/')]
        ...
        manifest = {
            ...
            'scope': 'all_companies',
            'database_scope': 'entire_database',
            'created_at': datetime.utcnow().isoformat() + 'Z',
            'database_type': 'sqlite' if db_uri.startswith('sqlite:') else 'postgresql',
            'includes': ['all_companies', 'all_users_and_company_relationships', 'database', 'application_files', 'uploaded_images', 'uploaded_files'],
            'restore_note': 'This is a complete system backup. It restores the entire database, all companies, users, company relationships, settings, transactions, and uploaded files.'
        }
        ...
        return jsonify({'success': True, 'message': 'Full POS backup created successfully.', 'filename': archive_name, 'download_url': f'/api/settings/download-backup/{archive_name}'})
...
def restore_full_backup():
    ...
    try:
        ...
        if manifest.get('scope') not in (None, 'all_companies'):
            return jsonify({'error': 'This archive is not a complete all-company POS backup'}), 400
        if db_uri.startswith(('postgresql:', 'postgres:')):
            parsed_uri = urlparse(db_uri)
            env = os.environ.copy()
            if parsed_uri.password:
                env['PGPASSWORD'] = parsed_uri.password
            restore_cmd = [find_psql(), f'--host={parsed_uri.hostname or "localhost"}', f'--port={parsed_uri.port or 5432}', f'--username={parsed_uri.username or "postgres"}', f'--dbname={parsed_uri.path.lstrip("/")}', '--file', database_path]
            ...
            target_db = _resolve_sqlite_database_path()
            if not target_db:
                return jsonify({'error': 'SQLite database path could not be resolved'}), 500
        ...
        restored_paths = []
        for relative_path in ('app/static/uploads', 'static/uploads', 'app/uploads'):
        ...
        if os.path.isdir(local_backups):
            shutil.copytree(local_backups, os.path.join(project_root, 'backups'), dirs_exist_ok=True)
            restored_paths.append('backups')
        external_uploads = os.path.join(extract_dir, 'uploads')
        if os.path.isdir(external_uploads):
            shutil.copytree(external_uploads, os.path.join(os.path.dirname(project_root), 'uploads'), dirs_exist_ok=True)
            restored_paths.append('uploads')
        db.engine.dispose()
        return jsonify({'success': True, 'message': 'Full POS backup restored successfully. Please refresh the application.', 'restored': ['database'] + restored_paths, 'manifest': manifest})
...
def get_backups():
    ...
    try:
        for filename in os.listdir(backup_dir):
            ...
            if filename.startswith('pos_full_backup_') and filename.endswith('.tar.gz') and os.path.isfile(filepath):
                stats = os.stat(filepath)
                backups.append({
                    'filename': filename,
                    'type': 'Full POS Backup',
                    'is_full': True,
            ...
            elif filename.endswith('.json') and f'company_{company_id}' in filename:
...
def delete_backup(filename):
    ...
    # Full POS archives are system-wide and may be deleted by authorized Settings users.
    if filename.startswith('pos_full_backup_') and filename.endswith('.tar.gz'):
        ...
        if os.path.isfile(backup_path):
            try:
                os.remove(backup_path)
                current_app.logger.info(f'Deleted full POS backup: {filename}')
                return jsonify({'success': True, 'message': 'Full POS backup deleted from the server'})
            except Exception as e:
                return jsonify({'error': str(e)}), 500
